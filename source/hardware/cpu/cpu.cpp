...
namespace nes
{
   ...
   Instruction Processor::PLP() noexcept
   {
      ...
      processor_status_ = memory_.read(0x0100 + stack_pointer_);
      co_return std::nullopt;
   }
   ...
   Instruction Processor::RTI() noexcept
   {
      ...
      program_counter = program_counter & 0xFF00 | pop();
      co_await std::suspend_always{};
   ...
   Instruction Processor::PLA() noexcept
   {
      ...
      // pull register from stack
      accumulator_ = memory_.read(0x0100 + stack_pointer_);
      co_return std::nullopt;
   }
   ...
   Instruction Processor::JMP_indirect() noexcept
   {
      ...
      Address const pointer_address{ assemble_address(pointer_address_low, pointer_address_high) };
      Data const low_address{ memory_.read(pointer_address) };
      co_await std::suspend_always{};

      // fetch PCH, copy latch to PCL
      program_counter = assemble_address(low_address, memory_.read(pointer_address + 1));
      co_return std::nullopt;
   }
   ...
   void Processor::EOR(Data const value) noexcept
   {
      ...
      // Z set if the accumulator is zero after EOR
      change_processor_status_flag(ProcessorStatusFlag::Z, not accumulator_);

      // N set if bit 7 of the accumulator is set after EOR
      change_processor_status_flag(ProcessorStatusFlag::N, accumulator_ & 0b1000'0000);
   }
   ...
   void Processor::ADC(Data const value) noexcept
   {
      ...
      change_processor_status_flag(ProcessorStatusFlag::V, ((accumulator_ ^ result) & (value ^ result) & 0b1000'0000) not_eq 0);

      // store result in accumulator
      accumulator_ = static_cast<Data>(result);

      // Z set if the accumulator is zero after ADC
      change_processor_status_flag(ProcessorStatusFlag::Z, not accumulator_);

      // N set if bit 7 of the accumulator is set after ADC
      change_processor_status_flag(ProcessorStatusFlag::N, accumulator_ & 0b1000'0000);
   }
   ...
   void Processor::LDY(Data const value) noexcept
   {
      ...
      // Z set if the Y register is zero after LDY
      change_processor_status_flag(ProcessorStatusFlag::Z, not y_);

      // N set if bit 7 of the Y register is set after LDY
      change_processor_status_flag(ProcessorStatusFlag::N, y_ & 0b1000'0000);
   }
   ...
   void Processor::LDX(Data const value) noexcept
   {
      ...
      // Z set if the X register is zero after LDX
      change_processor_status_flag(ProcessorStatusFlag::Z, not x_);

      // N set if bit 7 of the X register is set after LDX
      change_processor_status_flag(ProcessorStatusFlag::N, x_ & 0b1000'0000);
   }

   void Processor::CPY(Data const value) noexcept
   {
      // perform subtraction
      auto const result{ static_cast<Data>(y_ - value) };

      // set C if Y >= value
      change_processor_status_flag(ProcessorStatusFlag::C, y_ >= value);

      // Z set if result is zero
      change_processor_status_flag(ProcessorStatusFlag::Z, not result);

      // N set if bit 7 of result is set
      change_processor_status_flag(ProcessorStatusFlag::N, result & 0b1000'0000);
   }

   void Processor::CMP(Data const value) noexcept
   {
      // perform subtraction
      auto const result{ static_cast<Data>(accumulator_ - value) };
   ...
   void Processor::CPX(Data value) noexcept
   {
      // perform subtraction
      auto const result{ static_cast<Data>(x_ - value) };
   ...
   void Processor::SBC(Data value) noexcept
   {
      // perform subtraction
      auto result{ accumulator_ - value };
      ...
      change_processor_status_flag(ProcessorStatusFlag::C, result < 0x100);
      ...
      // Z set if the accumulator is zero after SBC
      change_processor_status_flag(ProcessorStatusFlag::Z, not accumulator_);

      // N set if bit 7 of the accumulator is set after SBC
      change_processor_status_flag(ProcessorStatusFlag::N, accumulator_ & 0b1000'0000);
   }
   ...
   Data Processor::ROL(Data value) noexcept
   {
      ...
      value = value << 1 | static_cast<Data>(old_carry);

      // Z set if the value is zero after ROL
      change_processor_status_flag(ProcessorStatusFlag::Z, not value);

      // N set if bit 7 of the value is set after ROL
      change_processor_status_flag(ProcessorStatusFlag::N, value & 0b1000'0000);
   ...
   Data Processor::LSR(Data value) noexcept
   {
      ...
      // Z set if the value is zero after LSR
      change_processor_status_flag(ProcessorStatusFlag::Z, not value);

      // N set if bit 7 of the value is set after LSR
      change_processor_status_flag(ProcessorStatusFlag::N, value & 0b1000'0000);
   ...
   Data Processor::ROR(Data value) noexcept
   {
      ...
      // Z set if the value is zero after ROR
      change_processor_status_flag(ProcessorStatusFlag::Z, not value);

      // N set if bit 7 of the value is set after ROR
      change_processor_status_flag(ProcessorStatusFlag::N, value & 0b1000'0000);
   ...
   Data Processor::DEC(Data value) noexcept
   {
      ...
      // Z set if the value is zero after DEC
      change_processor_status_flag(ProcessorStatusFlag::Z, not value);

      // N set if bit 7 of the value is set after DEC
      change_processor_status_flag(ProcessorStatusFlag::N, value & 0b1000'0000);
   ...
   Data Processor::INC(Data value) noexcept
   {
      ...
      // Z set if the value is zero after INC
      change_processor_status_flag(ProcessorStatusFlag::Z, not value);

      // N set if bit 7 of the value is set after INC
      change_processor_status_flag(ProcessorStatusFlag::N, value & 0b1000'0000);
